            ...
            <div class="alert alert-danger">
                <ul>
                    @foreach ($errors->all() as $error)
                        <li>{{ $error }}</li>
                        <a href="{{ route('admin.CateNewsManagement') }}" class="btn btn-outline-primary" style="height: fit-content;">Chuyển về trang Quản lý Danh Mục Tin Tức</a>
                    @endforeach
                </ul>
            </div>
        @endif
    <form action="{{ route('admin.catenew_Delete', $category_new->id)}}" method="post" enctype="multipart/form-data">
        @csrf
        @method('DELETE')
        <header class="bg-body p-2 d-flex justify-content-between mb-2 sticky-top z-1">
            <h2 class="">Kiểm tra danh mục category tin tức: {{ $category_new->id }} trước khi xóa</h2>
            <div>
                <button type="submit" name="post" id="post-btn" class="btn btn-primary" style="height: fit-content;">Thực hiện Xóa Danh Mục Tin Tức</button>
            ...
            <div class="col-3">
                <section class="bg-body rounded mb-3 p-2">
                    ...
                    <label for="fileUpload" class="form-label">
                        <img id="mainImage" src="{{ asset('assets/image_new/' . $category_new->image_url ?? 'img_phaceholder.jpg') }}" alt="ảnh nè" width="100%" class="img-thumbnail object-fit-contain mb-3">
                    </label>
