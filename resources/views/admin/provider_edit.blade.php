...
@section('main')
    ...
    <form action="{{ route('admin.providerEdit_update',$providers->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        <header class="bg-body p-2 d-flex justify-content-between mb-2 sticky-top z-1">
            <h2 class="">Sửa đối tác: {{$providers->id}}</h2>
            <div>
        ...
        <div class="row">
            ...
            <div class="col-9">
                <section class="bg-body rounded mb-3">
                    <input name="name" value="{{ old('name') ?? $providers->name}}" id="name" class="form-control form-control-lg" type="text" placeholder="Tên đối tác (không vượt quá 255 ký tự) " aria-label=".form-control-lg example">
                </section>
                ...
                <section class="bg-body rounded p-2 mb-3">
                    ...
                    <input type="email" name="email" value="{{ old('email') ?? $providers->email}}" id="email" class="form-control mb-3">
                    <div class="d-flex gap-3 justify-content-between">
                        <label for="password" class="h5">Mật khẩu</label>
                    </div>
                    <input type="password" name="password" value="{{ old('password') ?? $providers->password}}" id="password" class="form-control mb-3">
                    <div class="d-flex gap-3 justify-content-between">
                        <label for="phone" class="h5">Số điện thoại</label>
                    </div>
                    <input type="phone" name="phone" value="{{ old('phone') ?? $providers->phone}}" id="phone" class="form-control mb-3">
                    <!-- <div class="d-flex gap-3 justify-content-between">
                    ...
                    <input type="hidden" name="role" value="{{ old('role', 'provider') ?? $providers->role}}" id="role">
                </section>
