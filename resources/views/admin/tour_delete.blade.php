            ...
            <div class="alert alert-danger">
                <ul>
                    @foreach ($errors->all() as $error)
                        <li>{{ $error }}</li>
                        <a href="{{ route('admin.tourManagement') }}" class="btn btn-outline-primary" style="height: fit-content;">Chuyển về trang Quản lý Tour</a>
                    @endforeach
                </ul>
            </div>
        @endif
    <form action="{{ route('admin.tour_Delete', $tour->id) }}" method="POST" style="display:inline;">
        @csrf
        @method('DELETE')
        <header class="bg-body p-2 d-flex justify-content-between mb-2 sticky-top z-1">
            <h2 class="">Kiểm tra tour #{{ $tour->id }} trước khi xóa</h2>
            <div>
                <button type="submit" name="post" class="btn btn-primary" style="height: fit-content;">Thực Hiện Xóa Tour</button>
            </div>
        </header>
        <!-- <div class="alert alert-danger">
            <h4>Todo</h4>
