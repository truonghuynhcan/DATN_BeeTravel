...
@section('main')
    ...
    <form action="{{ route('admin.adminInsert_') }}" method="post" enctype="multipart/form-data">
        @csrf
        <header class="bg-body p-2 d-flex justify-content-between mb-2 sticky-top z-1">
            <h2 class="">Thêm admin mới</h2>
            <div>
        ...
        <div class="row">
            ...
            <div class="col-9">
                <section class="bg-body rounded mb-3">
                    <input name="name" value="{{ old('name') }}" id="name" class="form-control form-control-lg" type="text" placeholder="Tên admin (không vượt quá 255 ký tự) " aria-label=".form-control-lg example">
                </section>
                ...
                <section class="bg-body rounded p-2 mb-3">
                    ...
                    <!-- Hiển thị giá trị -->
                    <span class="form-control mb-3" id="role-display">{{ old('role', 'admin') }}
                    </span>

                    <!-- Lưu giá trị trong trường ẩn -->
                    <input type="hidden" name="role" value="{{ old('role', 'admin') }}" id="role">
                </section>
